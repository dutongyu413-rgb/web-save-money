...
export function SavingsFormPage() {
  ...
  const [mode, setMode] = useState<SavingsMode>(record?.amountCents && record.amountCents < 0 ? "take" : "add");
  ...
  return (
    <section className="screen form-screen">
      <ScreenHeader title={recordId ? "编辑储蓄" : "记储蓄"} backTo="/savings" />
      <form className="screen-scroll form-content" onSubmit={submit} noValidate>
        <div className="segmented" aria-label="储蓄操作类型">
          <button type="button" className={mode === "add" ? "active" : ""} onClick={() => setMode("add")}><Plus />增加储蓄</button>
          <button type="button" className={mode === "take" ? "active neutral" : ""} onClick={() => setMode("take")}><Minus />取用储蓄</button>
        </div>
        <div className="amount-field orange-focus">
        ...
        {mode === "take" && <p className="helper-note orange"><Info />这笔金额会减少本月净储蓄，储蓄率可能为负数。</p>}
        ...
        <div className="form-footer">
          <button className={`button full ${mode === "add" ? "savings-primary" : "primary"}`} disabled={submitting}>{submitting ? "正在保存" : "保存储蓄记录"}</button>
